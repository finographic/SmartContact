   ...
   /// if (typeof(user) === 'undefined') {
    ...
    ctxSip = {
        ...
        config : {
            ...
            password        : sipconfigdb.pwd,
            displayName     : sipconfigdb.end_point_ext,
            uri             : 'sip:'+ sipconfigdb.end_point_ext+'@'+ sipconfigdb.realm,
            wsServers       : sipconfigdb.wsserver,
            stunServers : [sipconfigdb.stunserver],
        ...
        newSession : function(newSess) {
            ...
            if (newSess.direction === 'incoming') {
                ...
                console.log('end point incomming');
        ...
        getUserMediaFailure : function(e) {
            ...
            ctxSip.setError(true, 'Media Error.', 'You must allow access to your microphone. <br /> Check the address bar.', true);
        },
        ...
        setStatus : function(status) {
            ...
            $(".sipstatus").html('<i class="fa fa-signal"></i> ' + status);
        },
        ...
        logItem : function(item) {
            ...
            i  = '<div class="list-group-item sip-logitem clearfix '+callClass+'" data-uri="'+item.uri+'" data-sessionid="'+item.id+'" title="">';
            ...
            $('#ddbell').append('<li><a href="#"><i class="fa fa-comment green-font"></i><span class="text">  INB call | ' + item.clid + ' | ' + moment(item.start).format('MM/DD hh:mm:ss a') + '</span><span class="timestamp">' + callLength + '</span></a></li>');
            }
            
            countnall += 1;
            $('.notifi').html(countnall);                    
       

            // Start call timer on answer
            if (item.status === 'answered') {
                console.log('Anserred');
            ...
            if (err === true) {
              ...
              // $("#mdlError").modal('show');
                ...
                if (closable) {
                    ...
                    $("#errormodal .modal-header").find('button').remove();
                    $("#errormodal .modal-title").html(title);
    ...
    ctxSip.phone.on('connected', function (e) {
        ...
        //randy click button to connect auto
        $('.connectbtncls').click();
    ...
    ctxSip.phone.on('disconnected', function(e) {
        ...
        //*Randy Here is where the phone disconnects. 
        ctxSip.setError(true, 'Websocket Disconnected.', 'An Error occurred connecting to the websocket. Please Close Window and try again');
    ...
    ctxSip.phone.on('invite', function (session) {
        console.log('invite session');

        session.mediaHandler.on('getDescription', function (sdpWrapper) {
            //sdpWrapper.sdp = sdpWrapper.sdp.replace('foo', 'bar');
            console.log('log edit here getDescription*');
        });
        
        
        session.mediaHandler.on('setDescription', function (sdpWrapper) {
           // console.log('****RandyChange here of Hack for a=setup:passive*');
            sdpWrapper.sdp = sdpWrapper.sdp.replace('a=setup:passive', 'a=setup:actpass');
            
            
            console.log(sdpWrapper.sdp);
